import os
# Setup environment (full fine-tuning 7b needs 160GB Memor
import torch
import pandas as pd
...
train_file = "data/data_for_llms/thermal_train_231.csv"
...
print(train_df.info())
print(test_df.info())
# print(test_df['articleInformation'][0])
# print(test_df['reactionDetails'][0])

instruction = '''Extract the reactionDetails from the mainReactions and articleInformation.'''

train_df['text'] = f'<s>[INST] {instruction} ' + train_df["articleInformation"] + " [/INST] " +  train_df["mainReactions"] +  " [/INST] " + train_df["reactionDetails"] +  "!!! </s>"
test_df['text'] = f'<s>[INST] {instruction} ' + test_df["articleInformation"] + " [/INST] " + test_df["mainReactions"] +  " [/INST] " + test_df["reactionDetails"] +  "!!! </s>"
...
model_name = "/public/home/tt/Llama/Meta-Llama-3.1-8B-Instruct"  # Path of the pretrained model downloaded from Hugging Face
new_model_dir = f"saved_models/self_Meta-Llama-3.1-8B-Instruct/train_{len(train_df)}_lr{learning_rate}_bs{per_device_train_batch_size}"  # Fine-tuned model name
...
# Save trained model
trainer.model.save_pretrained(new_model_dir, max_shard_size="10GB")
